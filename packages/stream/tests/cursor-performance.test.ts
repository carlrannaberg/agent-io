...
describe('Cursor Parser Performance', () => {
  ...
  it('should maintain constant memory usage', () => {
    ...
    for (let session = 0; session < 100; session++) {
      ...
      for (let i = 0; i < 100; i++) {
        cursorParser.parse(JSON.stringify({
          type: 'assistant',
          message: {
            role: 'assistant',
  ...
  it('should handle concurrent parsing efficiently', () => {
    ...
    const lines = [
      ...
      JSON.stringify({ type: 'user', message: { role: 'user', content: [{ type: 'text', text: 'Hi' }] }, session_id: 's1' }),
      JSON.stringify({ type: 'tool_call', subtype: 'started', call_id: 'c1', tool_call: { test: {} }, session_id: 's1' }),
      JSON.stringify({ type: 'assistant', message: { role: 'assistant', content: [{ type: 'text', text: 'OK' }] }, session_id: 's1' }),
      JSON.stringify({ type: 'result', subtype: 'success', duration_ms: 100, is_error: false, result: 'Done', session_id: 's1' })
    ];
