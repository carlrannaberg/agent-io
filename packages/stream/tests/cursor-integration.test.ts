...
describe('Cursor Parser Integration', () => {
  ...
  it('should handle streaming text assembly', () => {
    ...
    const resultEvents = parser.parse(
      JSON.stringify({
        type: 'result',
        subtype: 'success',
        duration_ms: 100,
  ...
  it('should handle multiple concurrent sessions', () => {
    ...
    parser.parse(
      JSON.stringify({
        ...
        message: {
          role: 'assistant',
          content: [{ type: 'text', text: 'Session2' }],
        },
    ...
    // Complete session 1
    const session1Result = parser.parse(
      JSON.stringify({
        type: 'result',
        subtype: 'success',
        duration_ms: 100,
    ...
    // Complete session 2
    const session2Result = parser.parse(
      JSON.stringify({
        type: 'result',
        subtype: 'success',
        duration_ms: 200,
  ...
  it('should handle malformed input gracefully', async () => {
    ...
    const validMsg = events.find(
      e => e.t === 'msg' && e.text === 'Valid message after errors',
    );
